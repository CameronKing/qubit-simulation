...
def runMultivaryJob(job_index):
    ...
    param_array = np.array(
        np.meshgrid(delta_var, delta_var, sigma_array, operating_points, indexing="ij")
    ).T.reshape(
...
def runSingleVaryJob(job_index):
    ...
    param_array1 = np.array(
        np.meshgrid(sigma_array, operating_points, indexing="ij")
    ...
    delta_array[: delta_var.shape[0], :] = np.array(
        [delta_var, np.ones(delta_var.shape[0])]
    ).T
    delta_array[delta_var.shape[0] :, :] = np.array(
        [np.ones(delta_var.shape[0]), delta_var]
    ).T
...
def ideal_job(job_index):
    ...
    indices = np.unravel_index(job_index, (len(operating_points), len(sigma_array)))
